 ...
 */
    ...
    public static String getPhoneDeviceCode() {
...
//            String str = string.substring(48, 50);
            ...
            } else if (Constants.Type_0B == i) {
                result = Constants.Type_0B_DESC;
            }
...
//        AAC501006A22EE0700000000000000005618B1F96D92837Ca1f9432b11b93e8bb4ae34539b7472c20eFD7b227469746c65223a2241494f2d454e54222c2272656d61726b223a226f6e65686f6d65222c22656e646f74797065223a2233222c22616363657074223a2230227db4DD
     ...
     */
    public static Boolean getDataIfForMe(String string, Context activity, String mPhoneDeviceCode) {
...
//        String datastring = "7B227469746C65223A2241494F2D454E54222C2272656D61726B223A2231E58FB7E58685E9959CE5AEA4222C22656E646F74797065223A2233222C22616363657074223A2231227D";
        ...
        if (!(("".equals(string)) && string.length() >= 179)) {
            String currentDeviceID = mPhoneDeviceCode.toUpperCase(); //记得大写
...
//        if ("9".equals(Received_Type)) {//妇科
//            Received_Type = "09";
//        }
//        if ("8".equals(Received_Type)) {//耳鼻喉
//            Received_Type = "08";
//        }
//        if ("7".equals(Received_Type)) {//一代一体机
//            Received_Type = "07";
//        }
//        if ("0".equals(Received_Type)) {//工作站
//            Received_Type = "00";
//        }
//        if ("1".equals(Received_Type)) {//HD3摄像机
//            Received_Type = "01";
//        }
...
//        String mSend_IDBy32 = MD5ChangeUtil.Md5_32(deviceId);
        ...
        LogUtils.e(TAG + "计算方法====CSString====" + "AA" + mLength + mRandom + mCMD_ID + mSend_Type + mSend_ID + mReceived_Type + mReceived_ID + mCMD + mData);


        LogUtils.e(TAG + "随机数========" + mRandom);
        LogUtils.e(TAG + "iPadID(MD5_32)===" + mSend_IDBy32);
        LogUtils.e(TAG + "iPadID(mSend_ID)===" + mSend_ID);
        LogUtils.e(TAG + "上位机类型(接收方设备类型)====" + mReceived_Type);
        LogUtils.e(TAG + "上位机ID(16)==" + Received_ID);
        LogUtils.e(TAG + "上位机ID(32)==" + Received_ID32);
        LogUtils.e(TAG + "命令CMD=======" + CMD);
...
//    }
     ...
     * 阿拉伯数字8==08  这里，全面做适配
     *
     * @param plf                   http请求过来的  十进制设备类型
     * @param mDeviceTypeDecNum     本地数据库存储的设备类型   10进制的
     * @return
     */
    public static boolean sameDevice(String plf, String mDeviceTypeDecNum) {
        if ("".equals(plf)) {
            return false;
        }
        if ("".equals(mDeviceTypeDecNum)) {
            return false;
        }
        /**
         * 因为接口中，设备类型是10进制，本地数据库存储的是16进制，而且我使用String存储的，所以每次都需要校准下（比如8和08，本质是一样的但是因为类型是String所以不同）
         *所以每次校验，把本地的16进制转换成十进制的字符串和接口传递的设备类型做校验
         */
        try {
            if (mDeviceTypeDecNum.equals(plf)) {
                return true;
            }
            //本地16进制的转换成十进制的

            String currentHttpType = "";
            switch (plf) {
                case "0":
                case "00":
                    currentHttpType = "00";
            ...
            if (currentHttpType.equals(mDeviceTypeDecNum)) {
                return true;
            }
            return false;
        } catch (Exception e) {
     ...
     * @param @return
     */
    public static String getHexHum(String hex) {
        switch (hex) {
            case "A":
            case "a":
                return "0A";
