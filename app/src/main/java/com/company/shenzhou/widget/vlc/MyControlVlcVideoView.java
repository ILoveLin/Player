...
import com.company.shenzhou.widget.PlayerView;
...
public class MyControlVlcVideoView extends RelativeLayout implements GestureDetector.OnGestureListener, View.OnClickListener {
    ...
    private MyControlVlcVideoView mPlayerControlView;
    ...
    private AppCompatImageView mTopBack;
    ...
    private ImageView mClearLogView;
...
//        mRootLayout.setOnTouchListener(mOnTouchVideoListener);
        ...
        //自新封装的控制布局
        mPlayerControlView = findViewById(R.id.player_control);
        ...
        mSettingView = findViewById(R.id.iv_right_setting);
        mClearLogView = findViewById(R.id.anim_clear_log);
        //错误提示
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.anim_clear_log: //清空日志
                break;
            case R.id.iv_right_setting: //打开日志View
                if (null != mListener) {
            ...
            case R.id.change_live: //清晰度
                showToast("清晰度");
                if (null != mListener) {
                    mListener.onClickChangeLive();
                }
                break;
            case R.id.pusher_mic:   //语音
                showToast("语音");
                if (null != mListener) {
                    mListener.onClickMic();
                }
                break;
            case R.id.photos:       //相册
                showToast("相册");
                if (null != mListener) {
                    mListener.onClickPhotos();
                }
                break;
...
//                                            FileUtil.RefreshAlbum(mTakeSnapshotFile.getAbsolutePath(), false, mContext);
     ...
     * 播放器的回调，以及点击事件的回调
     */
    private onPlayerOrClickListener mListener;

    public void setOnPlayerOrClickListener(MyControlVlcVideoView.onPlayerOrClickListener listener) {
        mListener = listener;
    }

    public interface onPlayerOrClickListener {
        /**
         * 开始播放
         */
        default void eventPlay() {
        }

        /**
         * 暂停播放和error差不多
         */
        default void eventStop() {
        }

        /**
         * 播放错误Stop差不多
         */
        default void eventError() {
        }
         ...
         * 设置的点击事件
         */
        default void onClickSetting() {
        }

        /**
         * 切换清晰度的点击事件
         */
        default void onClickChangeLive() {
        }

        /**
         * 麦克风推流的点击事件
         */
        default void onClickMic() {
        }

        /**
         * 打开相册的点击事件
         */
        default void onClickPhotos() {
        }

        /**
         * 打开相册的点击事件
         */
        default void onClickSnapShot() {
        }

        /**
         * 开始播放的点击事件
         */
        default void onClickRestart() {
        }
