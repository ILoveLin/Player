 ...
 * desc   : 设备界面
 */
public final class DeviceFragment extends TitleBarFragment<MainActivity> implements StatusAction {
    ...
    //Dialog信息录入开关, 默认没有填写完成==false
    private boolean isInputDataOk = false;
    private static final int REQUEST_CODE_SCAN_ONE = 0X01;
    //刷新界面列表数据
    private static final int Refresh_UI_Data = 0x10;
    //只刷新某个设备详细信息对话框的数据
    private static final int Refresh_MsgDialogData = 0x11;
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            switch (msg.what) {
                case Show_DeviceType_Dialog:       //显示设备类型对话
//                    showDeviceTypeDialog();
                    break;
                case Set_MsgDialogData:             //设置数据
//                    setMsgDialogData();
                    break;
                case Refresh_MsgDialogData:         //刷新数据
//                    refreshMsgDialogData();
                    break;
                case Refresh_UI_Data:               //刷新界面列表数据
                    showComplete();
...
//                        .setBackground(1,getActivity().getResources().getDrawable(R.drawable.icon_video_code))
                        .setListener((ListPopup.OnListener<String>) (popupWindow, position, s) -> {
                            if (s.equals(getResources().getString(R.string.device_scan))) {
                                ...
                                isInputDataOk = false;
                                //弹出类别对话框
                                mHandler.sendEmptyMessage(Show_DeviceType_Dialog);
                            } else {
    ...
    private void startThreadReadDBData() {
        new Thread(() -> {
            ...
            mHandler.sendEmptyMessage(Refresh_UI_Data);
        }).start();
    }
