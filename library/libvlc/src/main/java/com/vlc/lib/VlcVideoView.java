...
//        =======================注释不执行参数============================
        ...
        options.add(":file-caching=200");//文件缓存
        options.add(":live-cacheing=200");//直播缓存
        //options.add(":latency=10");//直播缓存
        options.add(":sout-mux-caching=200");//输出缓存
        options.add("--no-drop-late-frames");//关闭丢弃晚的帧 (默认打开)--"--drop-late-frames"
        options.add("--no-skip-frames");//关闭跳过帧 (默认打开)--"--skip-frames"
        options.add(":codec=mediacodec,iomx,all");//文件缓存
        options.add(":fullscreen");
        //如有出现图像失真或出现libvlc input source: attachment of directory-extractor failed...类似的错误，
        // 可以尝试添加"--demux=avformat"，或查看VLC官网议题：图像失真，尝试是否对你有利。
